...
def prepare_inputs(data):
    ...
    output0_len = np.ones_like(data["prompt"]).astype(
        np.uint32) * TOKENS_TO_GENERATE
    ...
    random_seed = RANDOM_SEED * \
        np.ones([input0_data.shape[0], 1]).astype(np.uint64)

    is_return_log_probs = IS_RETURN_LOG_PROBS * \
        np.ones([input0_data.shape[0], 1]).astype(bool)
